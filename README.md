...
아래 항목은 운영 수준의 추적성과 규제 대응 능력을 높이기 위한 권장 개발 순서입니다.

### 우선순위 1 — 변경 및 승인 추적성

- **구현됨:** 수정된 필드의 변경 전·후 값을 감사 로그에 함께 저장합니다.
- **구현됨:** 승인 취소 시 사유를 필수로 기록하고 기존 승인·취소 이력과 콘텐츠 해시를 보존합니다.
- 승인된 내용이 변경되면 기존 전자서명을 무효화하고 재승인을 요구합니다.
- 문서별 버전, 개정 사유, 작성자와 개정 시각을 조회하고 이전 버전과 비교합니다.

### 우선순위 2 — 운영 안정성과 감사 대응

- 관리자가 데이터 백업을 내려받고 검증된 백업으로 복구할 수 있게 합니다.
- 감사 로그를 날짜·사용자·행위별로 검색하고 Excel 또는 PDF로 내보냅니다.
- 감사 기록의 수정·삭제를 차단하고 조직 정책에 맞는 보존 기간을 설정합니다.
- 역할뿐 아니라 프로젝트와 객체 단위로 조회·작성·승인 권한을 세분화합니다.

### 우선순위 3 — 업무 편의성

- CAPA 기한 임박·초과, 승인 요청·반려, 시험 실패 알림을 제공합니다.
- 읽지 않은 알림 수와 담당자별 미처리 업무를 대시보드에 표시합니다.
- 프로젝트 진행률, 미승인 변경 요청, 미완료 CAPA, 위험 등급별 현황을 시각화합니다.

> 실제 의료기기 품질시스템에 적용하기 전에는 조직의 절차서, 전자기록·전자서명 정책 및 적용 규제 요구사항에 따라 별도의 검증이 필요합니다.
